...
async def generate_question_with_llm(topic: str, idx: int) -> dict:
...
async def generate_questions_for_topic_with_llm_batched(
    topic: str,
    num_questions: int,
    start_index: int,
    batch_size: int,
    ...
    while idx < start_index + num_questions:
        ...
        tasks = [generate_question_with_llm(topic, i) for i in range(idx, batch_end)]
...
async def generate_for_single_subtopic(state: State) -> dict:
    ...
    if prompt_class is None or not hasattr(prompt_class, subtopic):
        raise ValueError(
            f"No prompt string found for subtopic: {subtopic} in topic: {state.topic}"
        )
    prompt_str = getattr(prompt_class, subtopic)

    async def generate_question_with_custom_prompt(idx: int) -> dict:
        prompt = prompt_str.format(
            topic=subtopic, format_instructions=parser.get_format_instructions()
        )
        system_message = SystemMessage(content=prompt)
        response = await llm.ainvoke([system_message])
        try:
    ...
    while idx < state.target_count:
        ...
        tasks = [generate_question_with_custom_prompt(i) for i in range(idx, batch_end)]
        batch_results = await asyncio.gather(*tasks)
        questions.extend(batch_results)
    ...
    return {
        ...
        "c ubtopic_index": state.current_subtopic_index + 1,
    }


async def judge_diversity(state: State) -> dict:
    ...
    for subtopic in state.subtopics:
        subtopic_questions = [
            q for q in state.questions if q.get("subtopic") == subtopic
        ]
        unique_questions = []
        for q in subtopic_questions:
            if all(
                SequenceMatcher(None, q["question"], uq["question"]).ratio() < 0.8
                for uq in unique_questions
            ):
                unique_questions.append(q)
        filtered_questions.extend(unique_questions)
    return {**state.__dict__, "questions": filtered_questions}


async def check_completion_subtopics(state: State) -> dict:
    if state.current_subtopic_index >= len(state.subtopics):
        return {"next": "judge_diversity"}
    return {"next": "generate_for_single_subtopic"}


async def check_completion_single_topic(state: State) -> dict:
...
async def initialize_subtopics(state: State) -> dict:
    ...
    subtopics = [
        attr
        for attr in dir(prompt_class)
        if attr.isupper() and not attr.startswith("__")
    ]
    if not subtopics:
        raise ValueError(
            f"No ALL_CAPS subtopic attributes found for topic: {state.topic}"
        )
    return {
        **state.__dict__,
        "subtopics": subtopics,
